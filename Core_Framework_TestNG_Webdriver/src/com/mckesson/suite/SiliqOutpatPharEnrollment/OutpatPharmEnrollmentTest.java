package com.mckesson.suite.SiliqOutpatPharEnrollment;
...
public class OutpatPharmEnrollmentTest extends TestSuiteBase {
	...
	public void checkTestSkip(){
		...
		if(!TestUtil.isTestCaseRunnable(suite_SiliqOutpatPharmacyEnrollment__xls,this.getClass().getSimpleName())){
			APP_LOGS.debug("Skipping Test Case "+this.getClass().getSimpleName()+" as runmode set to NO");//logs
			throw new SkipException("Skipping Test Case "+this.getClass().getSimpleName()+" as runmode set to NO");//reports
		}
		// load the runmodes off the tests
		runmodes=TestUtil.getDataSetRunmodes(suite_SiliqOutpatPharmacyEnrollment__xls, this.getClass().getSimpleName());
	}
	
	
	@Test(dataProvider = "getTestDatasiliq")
	public void testRegistration(String Usersearch_Zipcode,String NPI,String FirstName,String LastName,String EmailAddress,String ConfirmEmailAddress,
			String UserID,String Password,
			String VerifyPassword,String FaxNumber,String PMOC,String NationalProviderIdentifier,String DEARegistrationNumber,
			String StateLicenseNumber,String TestCenter,String Phone,String Address,String State,String Inpat_Zip,
			String Prescriber_Signature5,String Prescriber_Date5,String Signature,String Pharmacyname,String Pharmacytype ) throws IOException, InterruptedException{
		APP_LOGS.debug("Starting the Prescriber Registration  test");
		openBrowser();
	    driver.get(CONFIG.getProperty("testSiteName"));
		getObject("siliq_Inpat_Phar_enroll_StartPharmacyButton").click();
	    driver.manage().window().maximize();
		APP_LOGS.debug("Entering the registration parameters");
		getObject("siliq_Inpat_Phar_enroll_Usersearch_Zipcode").sendKeys(Usersearch_Zipcode);
		getObject("siliq_Inpat_Phar_enroll_Usersearch_NPI").sendKeys(NPI);
		getObject("siliq_Inpat_Phar_enroll_Usersearch_FirstName").sendKeys(FirstName);
		getObject("siliq_Inpat_Phar_enroll_Usersearch_LastName").sendKeys(LastName);
		getObject("siliq_Inpat_Phar_enroll_Usersearch_SearchButton").click();
		Thread.sleep(3000);
		getObject("siliq_Inpat_Phar_enroll_Usersearch_NewUser").click();
		getObject("siliq_Inpat_Phar_enroll_CA_Firstname").sendKeys(FirstName);
		getObject("siliq_Inpat_Phar_enroll_CA_Lastname").sendKeys(LastName);
		getObject("siliq_Inpat_Phar_enroll_CA_Emailaddress").sendKeys(EmailAddress);
		getObject("siliq_Inpat_Phar_enroll_CA_Confirmemailaddress").sendKeys(ConfirmEmailAddress);
		getObject("siliq_Inpat_Phar_enroll_CA_Phonenumber").sendKeys(Phone);
		getObject("siliq_Inpat_Phar_enroll_CA_Username").sendKeys(UserID);
		getObject("siliq_Inpat_Phar_enroll_CA_Password").sendKeys(Password);
		getObject("siliq_Inpat_Phar_enroll_CA_ConfirmPassword").sendKeys(Password);
		Thread.sleep(30000);
		getObject("siliq_Inpat_Phar_enroll_CA_SubmitButton").click();
		Thread.sleep(3000);
		getObject("siliq_Inpat_Phar_enroll_ARintake_Confirmemailaddress").sendKeys(ConfirmEmailAddress);
		getObject("siliq_Inpat_Phar_enroll_ARintake_OfficeFax").sendKeys(FaxNumber);
		Select dropdown=new Select(getObject("siliq_Inpat_Phar_enroll_ARintake_Method_of_Contact"));
		dropdown.selectByVisibleText(PMOC);
		getObject("siliq_Inpat_Phar_enroll_ARintake_Nextbutton").click();
		getObject("siliq_Inpat_Phar_enroll_ARConfirmation_CertifyPharmacy").click();
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Pharmacyname").sendKeys(Pharmacyname);
		Select dropdown1=new Select(getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_PharmacyType"));
		dropdown1.selectByVisibleText(Pharmacytype);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Address").sendKeys(Address);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Zipcode").sendKeys(Inpat_Zip);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_DEA").sendKeys(DEARegistrationNumber);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_NPI").sendKeys(NationalProviderIdentifier);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_NCPDP").sendKeys(StateLicenseNumber);
		Thread.sleep(2000);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Nextbutton").click();
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Checkbox").click();
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Signature").sendKeys(FirstName+" "+LastName);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Signaturedate").click();
		Thread.sleep(4000);
		getObject("siliq_Inpat_Phar_enroll_PharmacyIntake_Submit").click();
	...
	public Object[][] getTestDatasiliq(){
		...
		return TestUtil.getData(suite_SiliqOutpatPharmacyEnrollment__xls, this.getClass().getSimpleName());
	}

	@AfterMethod
	public void reportDataSetResult(){
		if(skip)
			TestUtil.reportDataSetResult(suite_SiliqOutpatPharmacyEnrollment__xls, this.getClass().getSimpleName(), count+2, "SKIP");
		else if(fail){
			isTestPass=false;
			TestUtil.reportDataSetResult(suite_SiliqOutpatPharmacyEnrollment__xls, this.getClass().getSimpleName(), count+2, "FAIL");
		}
		else
			TestUtil.reportDataSetResult(suite_SiliqOutpatPharmacyEnrollment__xls, this.getClass().getSimpleName(), count+2, "PASS");
	...
	public void reportTestResult(){
		if(isTestPass)
			TestUtil.reportDataSetResult(suite_SiliqOutpatPharmacyEnrollment__xls, "Test Cases", TestUtil.getRowNum(suite_SiliqOutpatPharmacyEnrollment__xls,this.getClass().getSimpleName()), "PASS");
		else
			TestUtil.reportDataSetResult(suite_SiliqOutpatPharmacyEnrollment__xls, "Test Cases", TestUtil.getRowNum(suite_SiliqOutpatPharmacyEnrollment__xls,this.getClass().getSimpleName()), "FAIL");
